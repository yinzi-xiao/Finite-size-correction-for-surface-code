...
# ---------------------- plot p vs pf ------------------------
data = toolbox.load_simdata('zbias{}_5.json'.format(ETA))
run_case = data[0]['n_run']

# format plot
fig = plt.figure(figsize=(7, 5))
bax = brokenaxes(ylims=((0.255,0.285),(0.295,0.31),(0.42,0.525)),hspace=0.15,despine=True,diag_color='k')
plt.title('XZZX code, z-bias $\eta = $'+'{} noise'.format(ETA))
plt.xlabel('d')
bax.set_ylabel('$P_f$')
# bax.set_ylim(0.25, 0.55)
for p in P0_1:
    d = []
    p_f = []
    for run in data:
    ...
    bax.errorbar(d,p_f,p_f_error,fmt='o-',capsize=3,markersize=5,label='$p={}$'.format(p))
bax.legend(loc='best',columnspacing=0.2,frameon=True)
